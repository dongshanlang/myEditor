...
void MdiChild::newFile()
{
    ...
    //将当前文件命名为未命名的文档加编号，编号先使用再加1
    curFile = tr("未命名的文档%1.txt").arg(sequenceNumber++);

    //设置窗口标题,使用[*]可以在文档被更改后在文件名称后显示"*"号
    setWindowTitle(curFile+"[*]"+tr("-多文档编辑器"));

    //文档更改时发射contentsChanged()信号，执行doucumentsWasModified()槽
...
{
    //如果mabeSave()函数返回true，则关闭窗口，否则忽略该事件
    if(maybeSave())
    {
        event->accept();
    }
...
bool MdiChild::maybeSave()
{
    ...
    if(document()->isModified())
    {
        ...
        //弹出对话框
        box.exec();
        //如果用户选择是，则返回保存操作的结果，如果用户选取消，反返回false
...
{
    //canoicalFilePath()可以出去路径中的符号链接,"."和".."等符号
