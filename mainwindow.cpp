...
void MainWindow::updateWindowMenu()
{
    ...
    for(int i = 0;i <windows.size(); ++i)
    {
        ...
        //关联动作的出发信号到信号映射器的map()槽，这个槽会发射mapped()信号
...
{
    QSettings settings("DSLang", "myEditor");
...
void MainWindow::initWindow()
{
    ...
    label->setText(tr("<a href=\"https://dongshanlang.github.io\">东山郎</a>"));
    //标签文本为富文本
...
void MainWindow::on_actionSaveAs_triggered()
{
    if(activeMdiChild() && activeMdiChild()->saveAs())
        ui->statusBar->showMessage(tr("文件另存为保存成功"), 2000);
}
